...
This file owns correctness validation: local gates, AWS functional scenarios, runbooks, evidence checklists, and known limitations.

`docs/benchmarking.md` owns benchmark methodology, the benchmark snapshot schema, and the latest sanitized performance snapshot.

`docs/benchmark-history.jsonl` owns append-only sanitized performance records across runs. Do not duplicate timing tables in this file; link to the benchmark docs instead.
...
| P1 | Ranged ZIP extraction AWS rerun | Confirms the current no-disk ranged extraction path after the engine transition at 512, 1024, and 2048 MiB provider memory. | `benchmark-assets` `large-few` profile: `baseline` create, forced unchanged update, `sparse-changed` sparse update, destroy | 2026-05-02 | Pass; see `docs/benchmarking.md` for the latest timing, memory, and provider summary snapshot |
| P1 | ETag skip path AWS rerun | Confirms marker-free unchanged files are read through ranged ZIP entry streams, hashed with MD5, and skipped through destination `ETag` comparison without checksum `HeadObject` calls. | `benchmark-assets` forced unchanged `baseline` update via `RBD_BENCH_WAIT=false`; see `docs/benchmarking.md` | 2026-05-02 | Pass at 512, 1024, and 2048 MiB; provider Lambda invoked successfully |
| P1 | Changed-object overwrite AWS rerun | Confirms changed extracted files overwrite destination keys with plain `PutObject` under the CloudFormation custom-resource lifecycle. | `benchmark-assets` `large-few` `baseline` -> `sparse-changed` sparse update at 512, 1024, and 2048 MiB | 2026-05-02 | Pass |
...
```bash
RBD_BENCH_PROFILE=mixed RBD_BENCH_ASSET_SET=baseline RBD_BENCH_STACK_SUFFIX=Validation pnpm example deploy benchmark-assets
RBD_BENCH_PROFILE=mixed RBD_BENCH_ASSET_SET=baseline RBD_BENCH_STACK_SUFFIX=Validation RBD_BENCH_WAIT=false pnpm example deploy benchmark-assets
RBD_BENCH_PROFILE=mixed RBD_BENCH_ASSET_SET=sparse-changed RBD_BENCH_STACK_SUFFIX=Validation pnpm example deploy benchmark-assets
RBD_BENCH_PROFILE=mixed RBD_BENCH_ASSET_SET=files-removed RBD_BENCH_STACK_SUFFIX=Validation pnpm example deploy benchmark-assets
RBD_BENCH_STACK_SUFFIX=Validation pnpm example destroy benchmark-assets
```

The second `baseline` command toggles an inert property for the benchmark stack so CloudFormation invokes the provider against unchanged assets. Without that toggle, CDK reports no changes and the provider Lambda is not invoked.
